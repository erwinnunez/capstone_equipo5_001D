...
def list_pc(page: int = 1, page_size: int = 20,
            ...
            rut_cuidador: str | None = Query(None),
...
def get_pc(rut_paciente: str, rut_cuidador: str, db: Session = Depends(get_db)):
...
def update_pc(rut_paciente: str, rut_cuidador: str, payload: PacienteCuidadorUpdate, db: Session = Depends(get_db)):
    obj = svc.update(db, rut_paciente, rut_cuidador, payload)
    if not obj: raise HTTPException(404, "Not found")
    return obj

@router.delete("/{rut_paciente}/{rut_cuidador}")
def delete_pc(rut_paciente: str, rut_cuidador: str, db: Session = Depends(get_db)):
