...
def list_gp(page: int = 1, page_size: int = 20, rut_paciente: int | None = Query(None), db: Session = Depends(get_db)):
...
def get_gp(rut_paciente: int, db: Session = Depends(get_db)):
...
def update_gp(rut_paciente: int, payload: GamificacionPerfilUpdate, db: Session = Depends(get_db)):
    obj = svc.update(db, rut_paciente, payload)
    if not obj: raise HTTPException(404, "Not found")
    return obj

@router.delete("/{rut_paciente}")
def delete_gp(rut_paciente: int, db: Session = Depends(get_db)):
