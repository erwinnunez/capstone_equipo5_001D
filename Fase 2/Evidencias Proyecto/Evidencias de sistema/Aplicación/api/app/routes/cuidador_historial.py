...
router = APIRouter(prefix="/cuidador-historial", tags=["cuidador_historial"])

@router.get("", response_model=Page[CuidadorHistorialOut])
def list_hist(page: int = 1, page_size: int = 20, rut_cuidador: int | None = Query(None), db: Session = Depends(get_db)):
...
def create_hist(payload: CuidadorHistorialCreate, db: Session = Depends(get_db)):
