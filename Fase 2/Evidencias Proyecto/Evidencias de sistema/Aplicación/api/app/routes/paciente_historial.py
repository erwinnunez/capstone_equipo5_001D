...
router = APIRouter(prefix="/paciente-historial", tags=["paciente_historial"])

@router.get("", response_model=Page[PacienteHistorialOut])
def list_hist(page: int = 1, page_size: int = 20, rut_paciente: int | None = Query(None), db: Session = Depends(get_db)):
...
def create_hist(payload: PacienteHistorialCreate, db: Session = Depends(get_db)):
