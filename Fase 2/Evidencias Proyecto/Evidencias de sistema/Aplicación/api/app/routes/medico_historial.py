...
router = APIRouter(prefix="/medico-historial", tags=["medico_historial"])

@router.get("", response_model=Page[MedicoHistorialOut])
def list_hist(page: int = 1, page_size: int = 20, rut_medico: int | None = Query(None), db: Session = Depends(get_db)):
...
def get_hist(historial_id: int, db: Session = Depends(get_db)):
    obj = svc.get(db, historial_id)
    if not obj: raise HTTPException(404, "Not found")
    return obj

@router.post("", response_model=MedicoHistorialOut)
def create_hist(payload: MedicoHistorialCreate, db: Session = Depends(get_db)):
    return svc.create(db, payload)

@router.delete("/{historial_id}")
def delete_hist(historial_id: int, db: Session = Depends(get_db)):
