...
def list_evt(page: int = 1, page_size: int = 20, rut_paciente: int | None = Query(None), db: Session = Depends(get_db)):
...
def get_evt(id_evento: int, db: Session = Depends(get_db)):
    obj = svc.get(db, id_evento)
    if not obj: raise HTTPException(404, "Not found")
    return obj

@router.post("", response_model=EventoGamificacionOut, status_code=status.HTTP_201_CREATED)
def create_evt(payload: EventoGamificacionCreate, db: Session = Depends(get_db)):
    return svc.create(db, payload)

@router.delete("/{id_evento}")
def delete_evt(id_evento: int, db: Session = Depends(get_db)):
