...
class RangoPaciente(Base):
    ...
    id_parametro = Column(Integer, ForeignKey("parametro_clinico.id_parametro", ondelete="CASCADE"), nullable=False)
    ...
    definido_por = Column(Boolean, default=False, nullable=False)  # False = sistema

    paciente = relationship("Paciente", back_populates="rangos", lazy="joined")
    parametro = relationship("ParametroClinico", back_populates="rangos_paciente", lazy="joined")
